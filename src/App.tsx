...
const Navbar = () => (
  <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-xl border-b border-outline-variant/10">
    <nav className="flex justify-between items-center px-8 py-4 max-w-7xl mx-auto">
      <div className="text-xl font-bold tracking-tight text-primary font-headline">BeLinked</div>
      <div className="hidden md:flex items-center gap-8">
        {['Features', 'Architecture', 'Docs'].map((item) => (
          <a key={item} className="text-on-surface-variant hover:text-primary transition-colors font-mono text-sm" href={`#${item.toLowerCase()}`}>
            {item}
          </a>
        ))}
      </div>
